...
?   Avisos:
    -> As abas pintadas em AMARELO, são as abas a serem coletadas os valores.
    ...
    def unificar_dados(self):
        for planilha in self.planilhas_auxiliares:
            ...
            try:
                ...
                self.df_colunas_filtradas[destino] = df
...
"""
    def unificar_dados(self):
        for planilha in self.planilhas_auxiliares:
            ...
            df_auxiliar = pd.read_excel(caminho_planilha_auxiliar, sheet_name=aba)

            df_colunas_filtradas = df_auxiliar[colunas]

            #* Adiciona as informações filtradas dentro da planilha base. (ignore_index = não perder os valores)
            self.df_base = pd.concat([self.df_base, df_colunas_filtradas], ignore_index=True)
            """